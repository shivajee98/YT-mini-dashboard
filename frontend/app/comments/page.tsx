        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                <div className="space-y-2">
                    ...
                    />
                    <Button onClick={handleAddComment} className="flex items-center gap-2" disabled={commentsLoading || !newComment.trim()}>
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Button variant="ghost" size="sm" onClick={() => setReplyingTo(comment.id)} disabled={commentsLoading}>
                                                    Reply
                                                </Button>
                                                <Button variant="ghost" size="sm" onClick={() => handleDeleteComment(comment.id)} className="text-destructive hover:text-destructive" disabled={commentsLoading}>
                                                    <Trash2 className="h-4 w-4" />
                                                </Button>
                                                <span className="text-xs text-muted-foreground ml-auto">{comment.likeCount || 0} likes</span>
                                            </div>
                                            {replyingTo === comment.id && (
                                                <div className="mt-3 space-y-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <Button size="sm" onClick={() => handleAddReply(comment.id)} disabled={commentsLoading || !replyText.trim()}>
                                                            Reply
                                                        </Button>
                                                        <Button size="sm" variant="outline" onClick={handleCancelReply} disabled={commentsLoading}>
                                                            Cancel
                                                        </Button>
                                                    </div>
                                                </div>
                                                        ...
                                                        <div key={reply.id} className="flex items-start gap-2 p-2 bg-muted rounded">
                                                            ...
                                                            <div className="flex-1">
                                                                <div className="flex items-center gap-2 mb-1">
                                                                    ...
                                                                    <Button variant="ghost" size="sm" onClick={() => handleDeleteComment(reply.id)} className="text-destructive hover:text-destructive ml-auto" disabled={commentsLoading}>
